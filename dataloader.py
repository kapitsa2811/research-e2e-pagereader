...
class CSVDataset(Dataset):
    ...
    def __init__(self, train_file, class_list, transform=None):
        ...
        #self.alphabet = "abcdefghijklmnopqrstuvwxyz0123456789ç "
        ...
        self.image_names = sorted_nicely(list(self.image_data.keys()))
    ...
    def string_to_ids(self,transcription):
        ...
        for i in range(len(transcription)):
            if transcription[i] in self.alphabet:
...
class Resizer(object):
    ...
    def __call__(self, sample, min_side=1250, max_side=1760):
        image, annots = sample['img'], sample['annot']
        rows, cols, cns = image.shape
        smallest_side = min(rows, cols)

        # rescale the image so the smallest side is min_side
        scale = float(min_side) / smallest_side
        ...
        if largest_side * scale > max_side:
            scale = float(max_side) / largest_side
